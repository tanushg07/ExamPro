...
from .security import log_and_sanitize_error, sanitize_database_error
from .models import (
...
def edit_user(user_id):
    ...
    form = UserEditForm(current_user=current_user, target_user=user, obj=user)
    
    if form.validate_on_submit():
        # Security checks for privilege escalation prevention
        original_user_type = user.user_type
        new_user_type = form.user_type.data
        
        # Prevent non-super-admins from modifying admin users
        if user.user_type == 'admin' and not current_user.is_super_admin():
            flash('Access denied: Only super administrators can modify admin accounts.', 'danger')
            return redirect(url_for('main.admin_dashboard'))
        
        # Prevent privilege escalation to admin
        if new_user_type == 'admin' and not current_user.is_super_admin():
            flash('Access denied: Only super administrators can create or promote users to admin.', 'danger')
            return redirect(url_for('main.admin_dashboard'))
        
        # Prevent self-demotion for admins
        if user.id == current_user.id and original_user_type == 'admin' and new_user_type != 'admin':
            flash('Cannot demote your own admin account.', 'danger')
            return redirect(url_for('main.admin_dashboard'))
        ...
        try:
            ...
            # Only allow user type change if authorized
            if current_user.is_super_admin() or (original_user_type != 'admin' and new_user_type != 'admin'):
                user.user_type = new_user_type
            
            db.session.commit()
            
            # Log privilege changes for security monitoring
            ...
            db.session.rollback()
            flash(log_and_sanitize_error(e, "updating user", current_user.id, f"user_id: {user_id}"), 'danger')
...
def delete_user(user_id):
    ...
    if user.user_type == 'admin' and not current_user.is_super_admin():
        flash('Access denied: Only super administrators can delete admin accounts.', 'danger')
        return redirect(url_for('main.admin_dashboard'))
    
    if user.is_super_admin():
        flash('Cannot delete super administrator accounts.', 'danger')
        return redirect(url_for('main.admin_dashboard'))
...
def delete_exam(exam_id):
    ...
    try:
        ...
        db.session.rollback()
        current_app.logger.error(f'Error deleting exam {exam_id}: {str(e)}')
        flash(sanitize_database_error(e), 'danger')
    except Exception as e:
        db.session.rollback()
        flash(log_and_sanitize_error(e, "deleting exam", current_user.id, f"exam_id: {exam_id}"), 'danger')
...
def toggle_exam_publish(exam_id):
    exam = Exam.query.get_or_404(exam_id)
      try:
        ...
        db.session.rollback()
        current_app.logger.error(f'Error updating exam status for exam {exam_id}: {str(e)}')
        flash(sanitize_database_error(e), 'danger')
    except Exception as e:
        db.session.rollback()
...
@admin_required
def create_user():
    form = CreateUserForm(current_user=current_user)
    if form.validate_on_submit():
        try:
            # Security check for admin creation
            if form.user_type.data == 'admin' and not current_user.is_super_admin():
                flash('Access denied: Only super administrators can create admin accounts.', 'danger')
            ...
            db.session.rollback()
            flash(log_and_sanitize_error(e, "creating user", current_user.id), 'danger')
